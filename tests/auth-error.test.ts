...
describe("formatOtpSendError", () => {
  it("explains when the production redirect URL is not allowed", () => {
    expect(formatOtpSendError("Redirect URL is not allowed")).toBe(
      "登录链接发送失败：当前网站地址未加入 Supabase 的 Redirect URLs。请添加 https://www.zhivector.com 后保存。"
    );
  });
  ...
  it("explains the short resend cooldown", () => {
    expect(formatOtpSendError("For security purposes, you can only request this after 60 seconds.")).toBe(
      "登录链接发送过于频繁，请等待 60 秒后再试。"
    );
  });

  it("preserves an hourly or provider rate-limit message instead of claiming a 60-second cooldown", () => {
    expect(formatOtpSendError("Email rate limit exceeded")).toBe("登录链接发送失败：Email rate limit exceeded");
  });
});
