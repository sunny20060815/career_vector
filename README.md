...
1. 安装 Node.js 22 LTS。
2. 复制 `.env.example` 为 `.env.local`，填写变量。`DEEPSEEK_API_KEY`、`SUPABASE_SERVICE_ROLE_KEY` 和 `RESEND_API_KEY` 只能保存在本地或 Vercel 服务端环境变量中。`DEEPSEEK_ANSWER_MODEL` 保持为 `deepseek-v4-flash`，用于低延迟的非思考模式职业建议。
