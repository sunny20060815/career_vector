...
export function CareerWorkbench() {
  ...
  return (
    <main className="app-shell h-[100dvh] min-h-0 overflow-hidden bg-[#1f2a38] text-[#e8edf1]">
      <div className="grid h-full grid-cols-1 md:grid-cols-[248px_minmax(0,1fr)]">
        <aside className="hidden min-h-0 flex-col border-r border-[#425970] bg-[#243243] md:flex">
          ...
          <div className="px-4 pt-4"><button onClick={() => { setActiveView("planner"); setConversationId(undefined); setMessages([]); }} className="flex h-10 w-full items-center justify-center gap-2 border border-[#5e7d98] bg-[#2a3b4e] text-sm text-[#c5d1db] transition hover:border-[#428ecd] hover:text-white" type="button"><Plus size={15} />新建规划</button></div>
        ...
        <section className="relative flex min-h-0 min-w-0 flex-col overflow-hidden bg-[#202d3d]">
          <header className="shrink-0 border-b border-[#425970] bg-[#243243]/95">
            ...
            <nav className="flex h-11 items-stretch gap-6 border-t border-[#32455a] px-4 md:px-7" aria-label="主导航">
          ...
          {activeView === "planner" ? <>
            ...
            <form onSubmit={(event) => void submit(event)} className="pointer-events-auto mx-auto max-w-5xl border border-[#5b7792] bg-[#29394b] shadow-[0_-16px_50px_rgba(0, 0, 0,0.28)] focus-within:border-[#438dc9]">
              ...
              <div className="flex items-center justify-between border-t border-[#3a5066] px-4 py-2.5"><span className="text-[11px] text-[#547188]">Enter 发送 · Shift + Enter 换行</span><button disabled={loading || !question.trim()} className="grid h-9 w-9 place-items-center bg-[#387fb9] text-[#202b3a] transition hover:bg-[#63a6dd] disabled:cursor-not-allowed disabled:bg-[#4e6479] disabled:text-[#4c677d]" type="submit" aria-label="提交咨询"><ArrowUp size={17} /></button></div>
            </form>
...
function LoginOverlay() {
  ...
  return (
    <div ref={overlayRef} className="fixed inset-0 z-50 grid place-items-center overflow-y-auto bg-[#1c2534]/95 p-4 backdrop-blur-md">
      <div ref={panelRef} className="grid w-full max-w-4xl overflow-hidden border border-[#536e87] bg-[#243243] shadow-[0_28px_100px_rgba(0,0,0,0.65)] md:grid-cols-[0.9fr_1.1fr]">
        ...
        <div className="flex min-h-[460px] flex-col justify-center p-6 sm:p-10 md:p-12">
          ...
          {stage === "phone" ? (
            ...
            <form onSubmit={(event) => void verifyCode(event)} className="login-unit mt-8">
              ...
              <div className="mt-2 flex h-12 items-center border border-[#5b7792] bg-[#29394b] px-3 focus-within:border-[#4792cf]"><ShieldCheck size={17} className="mr-3 shrink-0 text-[#4c94cf]" /><input id="login-code" value={code} onChange={(event) => setCode(event.target.value.replace(/\D/g, "").slice(0, 8))} type="text" inputMode="numeric" autoComplete="one-time-code" required autoFocus placeholder="请输入短信验证码" className="h-full min-w-0 flex-1 border-0 bg-transparent text-sm text-white outline-none placeholder:text-[#42596b]" /></div>
              <button disabled={submitting || code.length < 4} className="mt-4 flex h-12 w-full items-center justify-center gap-2 bg-[#3b85c2] text-sm font-semibold text-[#202b3a] transition hover:bg-[#64a8df] disabled:cursor-not-allowed disabled:bg-[#4e6479] disabled:text-[#526e85]" type="submit">{submitting ? "正在登录…" : "验证并登录"}</button>
...
function MessageBlock({ message, showSuggestions, onSuggestedQuestion }: { message: UiMessage; showSuggestions: boolean; onSuggestedQuestion: (value: string) => void }) {
  ...
  if (message.role === "user") return <div ref={ref as React.RefObject<HTMLDivElement>} className="ml-auto max-w-3xl border border-[#536e87] bg-[#2b4058] px-4 py-3"><p className="text-[10px] font-semibold tracking-[0.14em] text-[#5097d2]">你的问题</p><p className="mt-2 whitespace-pre-wrap text-sm leading-7 text-[#dbe3e9]">{message.content}</p></div>;
  return <article ref={ref as React.RefObject<HTMLElement>} className="max-w-4xl border-l-2 border-[#d98560] bg-[#27384a] px-5 py-5"><div className="flex items-center gap-2 text-[10px] font-semibold tracking-[0.16em] text-[#d98c68]"><MessageSquareText size={14} />职业规划建议</div><p className="mt-3 whitespace-pre-wrap text-sm leading-8 text-[#c5d2dc]">{message.content}</p>{message.evidence && <Evidence evidence={message.evidence} />}{showSuggestions && message.suggestedQuestions?.length ? <div className="mt-5 border-t border-[#496177] pt-4"><p className="flex items-center gap-2 text-[11px] font-medium tracking-[0.08em] text-[#62a2d7]"><Sparkles size={13} />你可能还想问</p><div className="mt-3 flex flex-col gap-2 sm:flex-row sm:flex-wrap">{message.suggestedQuestions.map((question) => <button key={question} onClick={() => onSuggestedQuestion(question)} className="group flex min-h-10 w-full items-center justify-between gap-3 border border-[#536e87] bg-[#2a3b4e] px-3 py-2.5 text-left text-xs leading-5 text-[#a9bccb] transition hover:border-[#458eca] hover:text-white sm:w-auto" type="button"><span>{question}</span><ChevronRight size={13} className="shrink-0 text-[#496276] transition group-hover:translate-x-0.5 group-hover:text-[#60a4db]" /></button>)}</div></div> : null}</article>;
}
...
function ThinkingIndicator({ progress, preview }: { progress: ChatProgress | null; preview: EvidencePreview | null }) {
  ...
  return <div ref={rootRef} className="max-w-4xl overflow-hidden border border-[#526b83] bg-[#29394b]"><div className="relative h-0.5 overflow-hidden bg-[#32455a]"><i className="thinking-scan absolute left-0 top-0 h-full w-1/4 bg-[#4b97d6]" /></div><div className="flex items-center gap-4 px-5 py-5"><span className="flex h-7 items-end gap-1">{[0, 1, 2, 3].map((item) => <i key={item} className="thinking-bar block h-6 w-1 bg-[#4b97d6]" />)}</span><div><p ref={phraseRef} className="text-sm font-medium text-[#d8e0e7]">{thinkingPhrases[index]}</p><p className="mt-1 text-xs text-[#57768f]">{progress?.message ?? "正在检索招聘数据与技能关系..."}</p></div></div>{preview && <div className="border-t border-[#425970] px-5 pb-4"><ReferencePreview preview={preview} /></div>}</div>;
}

function Evidence({ evidence }: { evidence: ChatResponse["evidence"] }) {
  return <div className="mt-5"><div className="grid gap-px border border-[#496177] bg-[#496177] sm:grid-cols-3"><div className="bg-[#263647] p-3"><p className="text-[10px] tracking-[0.1em] text-[#547188]">识别技能</p><p className="mt-2 text-xs leading-5 text-[#b4c4d1]">{evidence.recognizedSkills.join("、") || "暂无"}</p></div><div className="bg-[#263647] p-3"><p className="text-[10px] tracking-[0.1em] text-[#547188]">预测目标年</p><p className="mt-2 text-xs text-[#b4c4d1]">{evidence.forecastYear}年</p></div><div className="bg-[#263647] p-3"><p className="text-[10px] tracking-[0.1em] text-[#547188]">直接观测组合</p><p className="mt-2 text-xs text-[#b4c4d1]">{evidence.observedPairCount}组</p></div></div><ReferencePreview preview={buildEvidencePreview(evidence)} /></div>;
}

function ReferencePreview({ preview }: { preview: EvidencePreview }) {
  return <div className="mt-4 border-t border-[#475f76] pt-3 text-xs leading-5 text-[#61839e]"><p className="font-medium text-[#5697cd]">本次已检索到的依据</p><p>技能：{preview.skills.join("、") || "暂无"}</p><p>职业：{preview.occupations.join("、") || "暂无"}</p><p>城市：{preview.cities.join("、") || "暂无"}</p><p className="mt-1 text-[#496277]">引用表：{preview.sources.join("、")}</p></div>;
}
