        ...
        <section className="relative flex min-h-0 min-w-0 flex-col overflow-hidden bg-[#03172d]">
          ...
          {activeView === "planner" ? <>
            <div ref={scrollRef} className="min-h-0 flex-1 overflow-y-auto px-4 pb-48 pt-6 md:px-8 md:pb-52 lg:px-12">
              {!messages.length && !loading ? <Intro audience={audience ?? "individual"} containerRef={introRef} onExample={(value) => { setQuestion(value); requestAnimationFrame(resizeTextarea); }} /> : <div className="mx-auto max-w-5xl space-y-7">{messages.map((message, index) => <MessageBlock key={message.id} message={message} showSuggestions={!loading && index === messages.length - 1} onSuggestedQuestion={(value) => void sendQuestion(value)} />)}{loading && <ThinkingIndicator progress={progress} preview={preview} question={[...messages].reverse().find((message) => message.role === "user")?.content ?? ""} />}</div>}
            </div>
