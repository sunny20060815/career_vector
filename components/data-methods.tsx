...
export function DataMethods() {
  ...
  return (
    <div ref={rootRef} className="mx-auto w-full max-w-6xl py-8 md:py-12">
      <section className="method-unit border-b border-[#1b2738] pb-8 md:pb-10">
      ...
      <section className="method-unit grid border-b border-[#1b2738] sm:grid-cols-2 lg:grid-cols-5">
        {dataStats.map((item) => <div key={item.label} className="border-b border-[#16202e] py-5 pr-5 sm:border-r lg:border-b-0 lg:pl-5 first:pl-0 last:border-r-0"><p className="font-serif text-2xl text-[#e9eef2]">{item.value}</p><p className="mt-2 text-xs font-medium text-[#8fa7bb]">{item.label}</p><p className="mt-1 text-[10px] text-[#4d687e]">{item.note}</p></div>)}
      </section>

      <section className="method-unit border-b border-[#1b2738] py-8 md:py-10">
        ...
        <div className="mt-6 grid gap-8 lg:grid-cols-[1.1fr_0.9fr]">
          ...
          <div className="border-l border-[#283b4a] pl-5">
            {timeline.map((item) => <div key={item.period} className="relative border-b border-[#162030] py-3 first:pt-0 last:border-b-0"><p className="font-mono text-xs text-[#539bd5]">{item.period}</p><p className="mt-1 text-sm text-[#d2dbe3]">{item.label}</p><p className="mt-1 text-xs leading-5 text-[#5f819c]">{item.detail}</p></div>)}
          </div>
        </div>
      </section>
      ...
      <section className="method-unit border-b border-[#1b2738] py-8 md:py-10">
        ...
        <div className="mt-6 grid gap-8 lg:grid-cols-[1fr_1fr]">
          ...
          <div className="grid grid-cols-2 gap-px bg-[#1a2637]"><QualityBlock value="87.76%" label="岗位技能识别覆盖率" note="至少识别一项技能" /><QualityBlock value="4.99项" label="岗位平均技能数" note="按全部原始岗位计算" /></div>
        </div>
      </section>

      <section className="method-unit border-b border-[#1b2738] py-8 md:py-10">
        <SectionHeading icon={Network} index="04" title="技能网络与组合价值" subtitle="识别企业实际共同要求的能力结构" />
        <div className="mt-6 grid gap-px bg-[#1a2637] md:grid-cols-2">
...
function SectionHeading({ icon: Icon, index, title, subtitle }: { icon: typeof Database; index: string; title: string; subtitle: string }) {
  return <div className="flex items-start gap-4"><span className="grid h-9 w-9 shrink-0 place-items-center border border-[#284257] text-[#549dd8]"><Icon size={17} /></span><div><p className="font-mono text-[10px] tracking-[0.16em] text-[#4e6980]">MODULE {index}</p><h2 className="mt-1 text-lg font-semibold text-[#e1e8ed]">{title}</h2><p className="mt-1 text-xs text-[#5f809b]">{subtitle}</p></div></div>;
}

function QualityBlock({ value, label, note }: { value: string; label: string; note: string }) {
  return <div className="bg-[#0b1017] p-5"><p className="font-serif text-2xl text-[#589ed8]">{value}</p><p className="mt-2 text-xs font-medium text-[#bdcbd7]">{label}</p><p className="mt-1 text-[10px] leading-4 text-[#537087]">{note}</p></div>;
}

function MethodBlock({ title, body }: { title: string; body: string }) {
  return <div className="bg-[#0a0e14] p-5 md:p-6"><p className="text-sm font-medium text-[#d4dde5]">{title}</p><p className="mt-3 text-xs leading-6 text-[#7290a8]">{body}</p></div>;
}
