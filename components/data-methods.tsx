...
export function DataMethods() {
  ...
  return (
    <div ref={rootRef} className="mx-auto w-full max-w-7xl py-10 md:py-14">
      <section className="method-unit border-b border-[#1b4a70] pb-8 md:pb-10">
        <p className="flex items-center gap-3 text-[10px] font-semibold tracking-[0.2em] text-[#73c6ff]"><span className="h-px w-10 bg-[#73c6ff]" />DATA &amp; METHODOLOGY</p>
        <h1 className="mt-5 font-serif text-4xl leading-tight text-white md:text-5xl">从招聘文本到职业决策信号</h1>
      ...
      <section className="method-unit border-b border-[#1b4a70] py-8 md:py-10">
        ...
        <div className="mt-6 grid gap-8 lg:grid-cols-[1fr_1fr]">
          ...
          <div className="grid grid-cols-2 gap-6"><QualityBlock value="87.76%" label="岗位技能识别覆盖率" note="至少识别一项技能" /><QualityBlock value="4.99项" label="岗位平均技能数" note="按全部原始岗位计算" /></div>
        </div>
      </section>

      <section className="method-unit border-b border-[#1b4a70] py-8 md:py-10">
        <SectionHeading icon={Network} index="04" title="技能网络与组合价值" subtitle="识别企业实际共同要求的能力结构" />
        <div className="mt-6 grid border-y border-[#1a4a72] md:grid-cols-2">
      ...
      <section className="method-unit py-8 md:py-10">
        ...
        <div className="mt-6 grid gap-8 lg:grid-cols-[1fr_1fr]">
          <div><p className="text-sm font-medium text-[#d8e0e7]">职业AI能力暴露度</p><p className="mt-3 text-sm leading-7 text-[#87a1b6]">依据O*NET 30.3的52项职业能力，由Kimi、DeepSeek、文心一言、豆包、智谱清言、Gemini、ChatGPT和Grok独立评估生成式AI对各项能力的参与程度，再结合职业对能力的重要性和水平要求加权。894个O*NET职业被映射至我国1,635个具体职业，并汇总到380个职业小类；歧义映射经过人工复核。</p><div className="mt-5 border-y border-[#245476] py-3 font-mono text-xs leading-6 text-[#91abc0]">AI暴露度 = 能力AI评分 × 能力重要性 × 水平要求的加权平均</div></div>
          <div><p className="text-sm font-medium text-[#d8e0e7]">AI与技能的互补和替代压力</p><p className="mt-3 text-sm leading-7 text-[#87a1b6]">平台比较2019—2022年与2023—2025年技能同AI技能的标准化共现强度，并使用2019—2021年职业结构作为固定权重。AI共现增强且技能需求上升，被识别为互补；AI共现增强但需求下降，被识别为潜在替代压力；共现增强而需求稳定，则进一步判断岗位内部是否发生任务重组。</p><div className="mt-5 flex items-start gap-3 border-y border-[#6d4b43] py-3"><TrendingUp size={16} className="mt-0.5 shrink-0 text-[#ee9870]" /><p className="text-xs leading-6 text-[#a4b7c7]">这些指标描述招聘市场中已经显现的能力组合与需求变化，不直接解释为人工智能造成的因果效应。</p></div></div>
        </div>
      </section>
    </div>
  );
...
function SectionHeading({ icon: Icon, index, title, subtitle }: { icon: typeof Database; index: string; title: string; subtitle: string }) {
  return <div className="flex items-start gap-4"><span className="mt-1 grid h-8 w-8 shrink-0 place-items-center text-[#73c6ff]"><Icon size={18} /></span><div><p className="font-mono text-[10px] tracking-[0.16em] text-[#58758b]">MODULE {index}</p><h2 className="mt-1 text-xl font-semibold text-[#e8f0f5]">{title}</h2><p className="mt-1 text-xs text-[#67869d]">{subtitle}</p></div></div>;
}

function QualityBlock({ value, label, note }: { value: string; label: string; note: string }) {
  return <div className="border-y border-[#245476] px-1 py-5"><p className="font-serif text-3xl text-[#73c6ff]">{value}</p><p className="mt-2 text-xs font-medium text-[#bdcbd7]">{label}</p><p className="mt-1 text-[10px] leading-4 text-[#5c7b91]">{note}</p></div>;
}

function MethodBlock({ title, body }: { title: string; body: string }) {
  return <div className="border-b border-[#173f5e] p-5 odd:md:border-r md:p-6"><p className="text-sm font-medium text-[#dce7ee]">{title}</p><p className="mt-3 text-xs leading-6 text-[#7894a9]">{body}</p></div>;
}
