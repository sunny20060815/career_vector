...
export function DataMethods() {
  ...
  return (
    <div ref={rootRef} className="mx-auto w-full max-w-6xl py-8 md:py-12">
      <section className="method-unit border-b border-[#475f76] pb-8 md:pb-10">
      ...
      <section className="method-unit grid border-b border-[#475f76] sm:grid-cols-2 lg:grid-cols-5">
      ...
      <section className="method-unit border-b border-[#475f76] py-8 md:py-10">
        ...
        <div className="mt-6 grid gap-8 lg:grid-cols-[1.1fr_0.9fr]">
          ...
          <div className="border-l border-[#536e87] pl-5">
...
function SectionHeading({ icon: Icon, index, title, subtitle }: { icon: typeof Database; index: string; title: string; subtitle: string }) {
  return <div className="flex items-start gap-4"><span className="grid h-9 w-9 shrink-0 place-items-center border border-[#58748e] text-[#549dd8]"><Icon size={17} /></span><div><p className="font-mono text-[10px] tracking-[0.16em] text-[#4e6980]">MODULE {index}</p><h2 className="mt-1 text-lg font-semibold text-[#e1e8ed]">{title}</h2><p className="mt-1 text-xs text-[#5f809b]">{subtitle}</p></div></div>;
}

function QualityBlock({ value, label, note }: { value: string; label: string; note: string }) {
  return <div className="bg-[#29394b] p-5"><p className="font-serif text-2xl text-[#589ed8]">{value}</p><p className="mt-2 text-xs font-medium text-[#bdcbd7]">{label}</p><p className="mt-1 text-[10px] leading-4 text-[#537087]">{note}</p></div>;
}

function MethodBlock({ title, body }: { title: string; body: string }) {
  return <div className="bg-[#263647] p-5 md:p-6"><p className="text-sm font-medium text-[#d4dde5]">{title}</p><p className="mt-3 text-xs leading-6 text-[#7290a8]">{body}</p></div>;
}
